...
int main(void)
{
	...
	LATBbits.LATB15 = 1; // Set all LEDs to off 
	LATBbits.LATB14 = 1;
    LATBbits.LATB13 = 1;
    LATBbits.LATB12 = 1;

	CNEN2bits.CN27IE = 1;   // enables change notification on SW1
    IEC1bits.CNIE = 1;	// enables change notifications to send to ISR 
    IFS1bits.CNIF = 0;  // sets flag to zero
	...
	while(1)
	{
		...
		if(IFS0bits.U1RXIF == 1) {	
			...
			LATBbits.LATB15 = 1;     // resetting LEDs to off after every time the LED to toggle is changed
			LATBbits.LATB14 = 1;
		    LATBbits.LATB13 = 1;
			LATBbits.LATB12 = 1;
...
// ******************************************************************************************* //
void __attribute__((interrupt)) _CNInterrupt(void){  // ISR function

    IFS1bits.CNIF = 0;     // resets flag to zero     
    
	if(PORTBbits.RB5 == 1){   // button released
        TMR1 = 0;              // resets timer
        PR1 = 14400;			// sets period to original rate
    }
    else if(PORTBbits.RB5 == 0) {   // button pressed
        TMR1 = 0;					// resets timer
        PR1 = 7200;   // sets period to half original rate in order to blink twice as fast
    }
}
